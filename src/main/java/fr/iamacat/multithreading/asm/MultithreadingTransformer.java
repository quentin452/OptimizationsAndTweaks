...
public class MultithreadingTransformer implements IClassTransformer {
    ...
    private static final Set<String> threadedFields = ImmutableSet.of("minX", "minY", "maxX", "maxY", "minZ", "maxZ");
                    ...
                    if (node.getOpcode() == Opcodes.GETFIELD || node.getOpcode() == Opcodes.PUTFIELD) {
                        ...
                        if (f.owner.equals("net/minecraft/block/Block")) {
                            ...
                            if (obfContains || devContains) {
                                transformed = true;
                                MultithreadingLogger.LOGGER.info(
                                    "Transforming threaded block data access in {}.{}()",
                                    transformedName,
                                    m.name);
                                f.owner = "org/embeddedt/archaicfix/block/ThreadedBlockData";
                                if (obfContains) f.name = threadedObfFields.get(f.name);
                                insns.previous();
                                if (isSetter) {
