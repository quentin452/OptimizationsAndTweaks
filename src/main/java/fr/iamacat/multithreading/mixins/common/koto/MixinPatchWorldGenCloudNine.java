...
public class MixinPatchWorldGenCloudNine extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int i, int j, int k) {
        if (world.getBlock(i, j, k) == spawnedOn && world.getBlock(i, j + 1, k) == Blocks.air && world.getBlock(i + space, j, k) == spawnedOn && world.getBlock(i + space, j, k + space) == spawnedOn && world.getBlock(i, j, k + space) == spawnedOn && world.getBlock(i + space, j + 1, k) == Blocks.air && world.getBlock(i + space, j + 1, k + space) == Blocks.air && world.getBlock(i, j + 1, k + space) == Blocks.air) {
