...
public class MixinGuiWorldSelection  extends GuiSelectWorld implements GuiYesNoCallback {
    ...
    public void drawString(FontRenderer par1FontRenderer, String text, int left, int top, int color, boolean unicodeFlag) {
        int scaleFactor = WorldSelectionAdvanced.getScaledResolution().getScaleFactor();
        if (scaleFactor % 2 == 0) {
