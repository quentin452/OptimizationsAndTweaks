        ...
        if (block != AtumBlocks.BLOCK_SAND) {
            ...
            if (block != AtumBlocks.BLOCK_SAND) {
                ...
                if (block != AtumBlocks.BLOCK_SAND) {
                    ...
                    block = world
                        .getBlock(par3 + width, world.getHeightValue(par3 + width, par5 + depth) - 1, par5 + depth);
                    if (block != AtumBlocks.BLOCK_SAND) {
                        return false;
                    } else {
                        ...
                        if (maxHeight - minHeight < 6) {
                            ...
                            for (papyrus = (int) (0.0F - treeCount - 6.0F); (float) papyrus
                                <= treeCount + 6.0F; ++papyrus) {
                                for (i = (int) (0.0F - chest - 6.0F); (float) i <= chest + 6.0F; ++i) {
                                    float x = (float) (papyrus * papyrus) / (treeCount * treeCount)
                                        + (float) (i * i) / (chest * chest);
                                    if (x <= 1.0F) {
                                        ...
                                        x = (float) (papyrus * papyrus) / ((treeCount + 4.0F) * (treeCount + 4.0F))
                                            + (float) (i * i) / ((chest + 4.0F) * (chest + 4.0F));
                                        z = world.getHeightValue(papyrus + par3, i + par5);
                                        if (world.getBlock(papyrus + par3, z - 1, i + par5) == AtumBlocks.BLOCK_SAND
                                            && x < 1.0F) {
                                            world.setBlock(
                                            ...
                                            if ((double) x < 0.3 && par2Random.nextInt(8) == 0) {
                                                for (y = -1; y <= 1; ++y) {
                                                    for (dx = -1; dx <= 1; ++dx) {
                                                        if (AtumBlocks.BLOCK_PAPYRUS.canBlockStay(
                                                            world,
                                                            par3 + papyrus + y,
                                                            z,
                                                            par5 + i + dx)) {
                        ...
                        for (int var26 = 0; var26 < 6; ++var26) {
                            ...
                            block = world
                                .getBlock(par3 + papyrus, world.getHeightValue(par3 + papyrus, par5 + i) - 1, par5 + i);
                            if (block == AtumBlocks.BLOCK_FERTILESOIL) {
                                (new WorldGenPalm(true, 5, 0, 0)).func_76484_a(
                                    world,
                                    par2Random,
                                    par3 + papyrus,
                                    world.getHeightValue(par3 + papyrus, par5 + i),
                                    par5 + i);
                                ++var24;
                            }
                        ...
                        for (i = 0; i < 10; ++i) {
                            ...
                            if (!var25 && block == AtumBlocks.BLOCK_FERTILESOIL) {
                                ...
                                TileEntity var29 = world.getTileEntity(
                                    par3 + var28,
                                    world.getHeightValue(par3 + var28, par5 + z),
                                    par5 + z);
                                AtumLoot.fillChest((IInventory) var29, 5, 0.2F);
                                var25 = true;
                            } else {
                                int dz;
                                if (!var27 && block.canSustainPlant(
                                    world,
                                    par3 + var28,
                                    y,
                                    par5 + z,
                                    ForgeDirection.UP,
                                    (IPlantable) AtumBlocks.BLOCK_PAPYRUS)) {
                                    for (dx = -1; dx <= 1; ++dx) {
                                        for (dz = -1; dz <= 1; ++dz) {
                                    ...
                                    for (dx = -1; dx <= 1; ++dx) {
                                        for (dz = -1; dz <= 1; ++dz) {
                                            ...
                                            Block belowBlock = world
                                                .getBlock(par3 + var28 + dx, currentY - 1, par5 + z + dz);
                                            Block currentBlock = world
                                                .getBlock(par3 + var28 + dx, currentY, par5 + z + dz);
                                            if (par2Random.nextInt(3) == 0 && belowBlock == AtumBlocks.BLOCK_FERTILESOIL
                                                && currentBlock == Blocks.air) {
                                                world.setBlock(
                                                    par3 + var28 + dx,
                                                    currentY,
                                                    par5 + z + dz,
                                                    AtumBlocks.BLOCK_FLAX,
                                                    13,
                                                    0);
                                            }
                                        }
