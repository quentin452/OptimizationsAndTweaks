    ...
    public ItemStack func_77659_a(ItemStack itemStack, World world, EntityPlayer player) {
        ...
        if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            if (!world.isRemote && world.getBlock(i, j, k) instanceof BlockLiquid) {
                b = new EntityBlockling(world, itemStack.stackTagCompound.getInteger("level"), itemStack.stackTagCompound.getInteger("xp"), itemStack.stackTagCompound.getInteger("upgrade"), itemStack.stackTagCompound.getInteger("special"), itemStack.stackTagCompound.getString("name"), player);
                ...
                b = new EntityBlockling(world, itemStack.stackTagCompound.getInteger("level"), itemStack.stackTagCompound.getInteger("xp"), itemStack.stackTagCompound.getInteger("upgrade"), itemStack.stackTagCompound.getInteger("special"), itemStack.stackTagCompound.getString("name"), player);
                b.setLocationAndAngles((float)i + 0.5F, j + 1, (float)k + 0.5F, MathHelper.wrapAngleTo180_float(world.rand.nextFloat() * 360.0F), 0.0F);
                b.onSpawnWithEgg(null);
                world.spawnEntityInWorld(b);
                world.playSoundAtEntity(b, "fireworks.twinkle", 1.0F, 1.0F);
                --itemStack.stackSize;
