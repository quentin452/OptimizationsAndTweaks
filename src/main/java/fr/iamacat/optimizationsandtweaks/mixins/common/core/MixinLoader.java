...
public class MixinLoader {
    ...
    {
        TreeMultimap<ModContainer, File> dupsearch = TreeMultimap.create(new Classers.ModIdComparator(), Ordering.arbitrary());
    ...
    {
        return optimizationsAndTweaks$instance().mods != null ? ImmutableList.copyOf(optimizationsAndTweaks$instance().mods) : ImmutableList.<ModContainer>of();
    }
    ...
    {
        return optimizationsAndTweaks$instance().namedMods.containsKey(modname) && optimizationsAndTweaks$instance().modController.getModState(optimizationsAndTweaks$instance.namedMods.get(modname))!= LoaderState.ModState.DISABLED;
    }
    @Shadow
