...
public class MixinPatchSpawnerAnimals {
    ...
    @Unique
    private boolean shouldSpawnCreatureType(WorldServer p_77192_1_, boolean peaceful, boolean hostile, boolean animals, EnumCreatureType creatureType) {
        return ((!creatureType.getPeacefulCreature() || hostile) &&
            (creatureType.getPeacefulCreature() || peaceful) &&
            (!creatureType.getAnimal() || animals) &&
            p_77192_1_.countEntities(creatureType, true) <= creatureType.getMaxNumberOfCreature() * this.multithreadingandtweaks$eligibleChunksForSpawning.size() / 256);
    }
    ...
    private int spawnCreatures(WorldServer p_77192_1_, ChunkCoordinates spawnPoint, EnumCreatureType creatureType) {
        ...
        List<ChunkCoordIntPair> eligibleChunks = new ArrayList<>(multithreadingandtweaks$eligibleChunksForSpawning.keySet());
    ...
    @Unique
    private int spawnEntitiesInChunk(WorldServer p_77192_1_, ChunkCoordinates spawnPoint, EnumCreatureType creatureType, int j1, int k1, int l1) {
        int i2 = 0;
    ...
    @Unique
    private int attemptSpawnEntity(WorldServer p_77192_1_, EnumCreatureType creatureType, int k2, int k1, int i3, BiomeGenBase.SpawnListEntry spawnlistentry, IEntityLivingData ientitylivingdata, ChunkCoordinates spawnPoint) {
    ...
    @Unique
    private int spawnEntityAndHandleEvents(WorldServer p_77192_1_, EnumCreatureType creatureType, int k2, int k1, int i3, BiomeGenBase.SpawnListEntry spawnlistentry, IEntityLivingData ientitylivingdata) {
        ...
        Event.Result canSpawn = ForgeEventFactory.canEntitySpawn(entityliving, p_77192_1_, k2 + 0.5F, (float) k1, i3 + 0.5F);
