...
public abstract class MixinEntityItem extends Entity {
    ...
    public void onUpdate() {
        ...
        if (optimizationsAndTweaks$handleEntityItemUpdate(stack)) {
            return;
        }

        optimizationsAndTweaks$handleEntityItem(stack);
    }
    @Unique
    private boolean optimizationsAndTweaks$handleEntityItemUpdate(ItemStack stack) {
        return stack != null && stack.getItem() != null && stack.getItem().onEntityItemUpdate(entityItem);
    }
    @Unique
    private void optimizationsAndTweaks$handleEntityItem(ItemStack stack) {
        if (this.getEntityItem() == null) {
            this.setDead();
        } else {
            optimizationsAndTweaks$performEntityItemUpdate();

            if (this.delayBeforeCanPickup > 0) {
                --this.delayBeforeCanPickup;
            }

            optimizationsAndTweaks$moveEntity();

            optimizationsAndTweaks$handleMotion();

            optimizationsAndTweaks$updateAgeAndLifespan(stack);
        }
    }
    @Unique
    private void optimizationsAndTweaks$performEntityItemUpdate() {
        super.onUpdate();
    ...
    @Unique
    private void optimizationsAndTweaks$moveEntity() {
        this.moveEntity(this.motionX, this.motionY, this.motionZ);
        boolean flag = (int)this.prevPosX != (int)this.posX || (int)this.prevPosY != (int)this.posY || (int)this.prevPosZ != (int)this.posZ;

        if (flag || this.ticksExisted % 25 == 0) {
            optimizationsAndTweaks$handleLavaMotion();
            searchForOtherItemsNearby();
        }
    }
    @Unique
    private void optimizationsAndTweaks$handleLavaMotion() {
        if (this.worldObj.getBlock(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ)).getMaterial() == Material.lava) {
            // Lava motion handling logic from the original method
    ...
    @Unique
    private void optimizationsAndTweaks$handleMotion() {
        float f = 0.98F;
    ...
    @Unique
    private void optimizationsAndTweaks$updateAgeAndLifespan(ItemStack item) {
        ++this.age;
