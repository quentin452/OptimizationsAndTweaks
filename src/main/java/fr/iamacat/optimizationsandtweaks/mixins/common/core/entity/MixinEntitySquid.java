...
public class MixinEntitySquid extends EntityWaterMob {
    ...
    public void onLivingUpdate() {
        ...
        if (this.squidRotation > ((float) Math.PI * 2F)) {
            ...
            if (this.getRNG()
                .nextInt(10) == 0) {
                this.rotationVelocity = 1.0F / (this.getRNG()
                    .nextFloat() + 1.0F) * 0.2F;
            }
        }
        ...
        if (this.isInWater()) {
            ...
            this.renderYawOffset += (-((float) FastMath.atan2(this.motionX, this.motionZ)) * 180.0F / (float) Math.PI
                - this.renderYawOffset) * 0.1F;
            this.rotationYaw = this.renderYawOffset;
            this.squidYaw += (float) Math.PI * this.field_70871_bB * 1.5F;
            this.squidPitch += (-((float) FastMath.atan2(f, this.motionY)) * 180.0F / (float) Math.PI - this.squidPitch)
                * 0.1F;
        } else {
