...
public abstract class MixinPFQueue implements Runnable {
     ...
     */
    @Overwrite
    public static boolean tryPath(Entity entity, int x, int y, int z, float var2, int priority, IPFCallback parCallback,
        ChunkCoordinatesSize parCoordSize) {
    ...
    @Unique
    private static PFJobData optimizationsAndTweaks$createPFJob(Entity entity, int x, int y, int z, float var2,
        IPFCallback parCallback, ChunkCoordinatesSize parCoordSize) {
        PFJobData job;
        if (entity != null) {
