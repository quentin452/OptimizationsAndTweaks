...
public class MixinMainDisastercraft {
    ...
    public void preInit(FMLPreInitializationEvent e) {
        ...
        agapanthus = (new Agapanthus()).setHardness(0.0F).setStepSound(Block.soundTypeGrass).setBlockName("agapanthus").setBlockTextureName("disastercraft:Agapanthus");
        GameRegistry.registerBlock(agapanthus, "Agapanthus");
        brambles = (new Brambles()).setHardness(0.0F).setStepSound(Block.soundTypeGrass).setBlockName("brambles").setBlockTextureName("disastercraft:Brambles");
        GameRegistry.registerBlock(brambles, "Brambles");
        boiled = (new Boiled(5, 0.3F, false)).setPotionEffect(Potion.confusion.id, 17, 0, 0.8F).setTextureName("disastercraft:Boiled").setUnlocalizedName("boiled");
