...
public class MixinNetherWorldGenGanys {
            ...
            for(int x = 0; x < 16; ++x) {
                for(int y = 20; y < 84; ++y) {
                    for(int z = 0; z < 16; ++z) {
                        ...
                        if (GanysNether.shouldGenerateCrops && rand.nextInt(GanysNether.netherCropRate) == 0 && this.shouldGenerate(world, blockX, blockY, blockZ) && this.hasLavaNearby(world, blockX, blockY - 1, blockZ)) {
                            switch (rand.nextInt(6)) {
                                ...
                                case 2:
                                    ...
                                    return;
                                case 3:
                                    if (GanysNether.shouldGenerateWitherShrub && rand.nextInt(GanysNether.witherShrubRate) == 0) {
                                        world.setBlock(blockX, blockY - 1, blockZ, ModBlocks.tilledNetherrack);
    ...
    private boolean shouldGenerate(World world, int x, int y, int z) {
        return y >= 25 && !world.isAirBlock(x, y - 1, z) && world.getBlock(x, y - 1, z).getMaterial() != Material.lava && world.isAirBlock(x, y, z) && world.isAirBlock(x, y + 1, z) && world.isAirBlock(x, y + 2, z);
    }
    @Shadow
