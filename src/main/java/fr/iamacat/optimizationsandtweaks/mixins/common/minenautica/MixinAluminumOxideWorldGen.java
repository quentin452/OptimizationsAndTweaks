...
public class MixinAluminumOxideWorldGen implements IWorldGenerator {
            ...
            for (int iteration = 0; iteration < DIRECTIONS.length && !isValidPosition; iteration++) {
                ...
                if ((biome instanceof BiomeGenJungle || biome instanceof BiomeGenDesert)
                    && this.makeVein(
                        ...
                        random.nextInt(2) + 3,
                        Blocks.stone,
                        BlocksAndItems.aluminumOxideOre,
                        random)
                    && Xpos >= chunkX * CHUNK_SIZE
                    && Xpos < (chunkX + 1) * CHUNK_SIZE
                    && Zpos >= chunkZ * CHUNK_SIZE
                    && Zpos < (chunkZ + 1) * CHUNK_SIZE) {
     ...
     */
    @Overwrite(remap = false)
    private boolean makeVein(World world, int x, int y, int z, int veinSize, Block blockToReplace, Block blockToPlace,
        Random random) {
        ...
        for (int blocksNeeded = veinSize - 1; blocksNeeded > 0; --blocksNeeded) {
            ...
            if (optimizationsAndTweaks$isValidPosition(
                world,
                coords[0],
                coords[1],
                coords[2],
    ...
    @Unique
    private boolean optimizationsAndTweaks$isValidPosition(World world, int x, int y, int z,
        Set<String> generatedBlocks, Block blockToReplace) {
