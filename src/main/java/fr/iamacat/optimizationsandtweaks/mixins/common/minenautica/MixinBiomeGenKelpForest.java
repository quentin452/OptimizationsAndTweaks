...
public abstract class MixinBiomeGenKelpForest extends BiomeGenBase {
    ...
    public MixinBiomeGenKelpForest(int p_i1971_1_) {
        super(p_i1971_1_);
    }
                ...
                if (l1 <= 0 + random.nextInt(5)) {
                    ...
                    if (block2 != null && block2.getMaterial() != Material.air) {
                        if (block2 == Blocks.stone) {
                            if (k == -1) {
                                replacableBlock[i2] = BlocksAndItems.kelpForestGrass;
                                replace = true;
                                below = 0;
                                k = 0;
                            } else if (k > 0) {
                        ...
                        if (below == 0) {
                            ++below;
                        } else if (replacableBlock[i2] == BlocksAndItems.kelpForestGrass) {
                            below = 1;
                        } else if (below >= 1 && below < 6) {
                            int mossy = random.nextInt(100);
                            if (mossy >= 25) {
                                replacableBlock[i2] = BlocksAndItems.kelpForestRock;
                            } else {
                                replacableBlock[i2] = BlocksAndItems.kelpForestRockMossy;
                            }
