...
public class MixinNetherliciousEventHandler {
    ...
    public void livingUpdate(LivingEvent.LivingUpdateEvent event) {
        ...
        if (!entity.worldObj.isRemote && !entity.isSneaking() && entity.onGround) {
            optimizationsAndTweaks$checkBlockEffects(
                entity,
                x,
                y,
                z,
                ModBlocks.MagmaBlock,
                HOT_FLOOR,
                EntityPiglin.class,
                EntityHoglin.class);

            optimizationsAndTweaks$checkBlockDamageAndPotion(
                entity,
                x,
                y,
                z,
                ModBlocks.BlackstoneVent,
                4,
                9,
                HOT_FLOOR);
    ...
    @Unique
    private void optimizationsAndTweaks$checkBlockEffects(Entity entity, double x, double y, double z, Block block,
        DamageSource damageSource, Class<?>... excludedEntities) {
        if (!optimizationsAndTweaks$isExcludedEntity(entity, excludedEntities)
            && entity.worldObj.getBlock(MathHelper.floor_double(x), (int) (y - 0.45), MathHelper.floor_double(z))
                == block) {
    ...
    @Unique
    private void optimizationsAndTweaks$checkBlockDamageAndPotion(Entity entity, double x, double y, double z,
        Block block, int damageMeta, int potionMeta, DamageSource damageSource) {
        if (entity.worldObj.getBlock(MathHelper.floor_double(x), (int) (y - 0.45), MathHelper.floor_double(z)) == block
            && entity.worldObj
                .getBlockMetadata(MathHelper.floor_double(x), (int) (y - 0.45), MathHelper.floor_double(z))
                == damageMeta) {
            entity.attackEntityFrom(damageSource, 1.0F);
        }
    }
    ...
    @Unique
    private void optimizationsAndTweaks$checkBlockPotion(Entity entity, double x, double y, double z, Block block,
        int potionMeta, Potion potion, int duration) {

        if (entity instanceof EntityLivingBase) {

            EntityLivingBase livingEntity = (EntityLivingBase) entity;

            if (entity.worldObj.getBlock(MathHelper.floor_double(x), (int) (y - 0.45), MathHelper.floor_double(z))
                == block
                && entity.worldObj
                    .getBlockMetadata(MathHelper.floor_double(x), (int) (y - 0.45), MathHelper.floor_double(z))
                    == potionMeta) {
