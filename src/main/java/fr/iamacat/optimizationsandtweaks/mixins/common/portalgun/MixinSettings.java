...
public class MixinSettings {
    ...
    public static void handlePortalGunRecipe(boolean easyMode) {
        ...
        if (PortalGun.getSettings("hardModePortalGun") != 1) {
            GameRegistry.addRecipe(new ItemStack(PortalGun.itemPGBlue, 1, 0), "X##", "DC#", "#X#", '#', Items.iron_ingot, 'X', Blocks.obsidian, 'D', Items.diamond, 'C', easyMode ? Items.ender_pearl : PortalGun.itemMiniBlackHole);
            GameRegistry.addRecipe(is, "# #", "DRD", "# #", '#', Items.iron_ingot, 'R', PortalGun.itemMiniBlackHole, 'D', Items.diamond);
        } else {
            for (Integer integer : enabledChestLoot) {
                String s = getLootOption(integer);
                if (!s.equalsIgnoreCase("")) {
                    ChestGenHooks.getInfo(s).removeItem(new ItemStack(PortalGun.itemPGBlue, 1, 32767));
                    ChestGenHooks.getInfo(s).removeItem(new ItemStack(PortalGun.itemPortalSpawner, 1, 32767));
                }
            }

            GameRegistry.addRecipe(is, "# #", "DRD", "# #", '#', Items.iron_ingot, 'R', new ItemStack(PortalGun.itemPGBlue, 1, 0), 'D', Items.diamond);
        }
    ...
    public static void handleLFBRecipe(boolean easyMode) {
        ...
        if (easyMode) {
            GameRegistry.addRecipe(new ItemStack(PortalGun.itemLFB, 1), "# #", "#O#", "#O#", '#', Items.iron_ingot, 'O', Blocks.obsidian);
        } else {
            GameRegistry.addRecipe(new ItemStack(PortalGun.itemLFB, 1), "#O#", "#D#", "#D#", '#', Items.iron_ingot, 'O', Blocks.obsidian, 'D', Items.diamond);
        }
    }
    /**
     * @author
