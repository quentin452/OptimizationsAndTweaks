...
public class MixinPPAPEventHandler {
    ...
    public void onMobDrops(LivingDropsEvent event) {
        ...
        if (event.entity instanceof EntityMob && rand.nextInt(100) < probability) {
            ...
            if (sword.getToolMaterialName()
                .equals(ModItems.ppapToolMaterial.name())) {
                event.drops.add(
                    new EntityItem(
                        ...
                        event.entity.posY,
                        event.entity.posZ,
                        new ItemStack(ModItems.record)));
            } else if (sword.getToolMaterialName()
                .equals(ModItems.ppapToolMaterialBig.name())) {
                    event.drops.add(
                        new EntityItem(
                            event.entity.worldObj,
                            event.entity.posX,
                            event.entity.posY,
                            event.entity.posZ,
                            new ItemStack(ModItems.recordLong)));
                }
        }
    }
}
