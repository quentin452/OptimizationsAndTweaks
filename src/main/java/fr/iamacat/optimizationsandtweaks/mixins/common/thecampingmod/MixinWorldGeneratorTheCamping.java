...
public abstract class MixinWorldGeneratorTheCamping {
    ...
    public void generateSurface(World world, Random random, int blockX, int blockZ) {
        if (Objs$.MODULE$.config()
            .worldGenHemp()) {
            int hempMulti = Math.max(
                1,
                Objs$.MODULE$.config()
                    .hempGenMulti());
    ...
    private void optimizationsAndTweaks$generateCampsite(World world, Random random, int blockX, int blockZ) {
        ...
        BiomeGenBase.TempCategory tempCategory = world.getBiomeGenForCoords(checkX, checkZ)
            .getTempCategory();
    ...
    @Unique
    private boolean optimizationsAndTweaks$isValidSpawnArea(World world, int x, int y, int z, int xLength,
        int zLength) {
    ...
    private void optimizationsAndTweaks$configureTentTileEntity(World world, int x, int y, int z) {
        try {
            ...
            if (tileEntity != null) {
                java.lang.reflect.Method setRotation = tileEntity.getClass()
                ...
                Object module = tileEntityTentClass.getField("MODULE$")
                    .get(null);
                java.lang.reflect.Method bedsMethod = tileEntityTentClass.getMethod("BEDS");
                Object beds = bedsMethod.invoke(module);

                java.lang.reflect.Method setContends = tileEntity.getClass()
                    .getMethod("setContends", int.class, Object.class, boolean.class, int.class);
    ...
    private void optimizationsAndTweaks$spawnCamper(World world, int x, int y, int z) {
        try {
            ...
            java.lang.reflect.Constructor<?> constructor = camperClass
                .getConstructor(World.class, int.class, int.class, int.class);
    ...
    @Unique
    private boolean optimizationsAndTweaks$generateSingleHemp(World world, Random random, int baseX, int baseY,
        int baseZ) {
        ...
        if (!Objs$.MODULE$.hemp()
            .canBlockStay(world, targetX, baseY, targetZ)) {
            return true;
        }
