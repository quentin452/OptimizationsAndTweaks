...
public class MixinGenericEventsWitchery {
    ...
    /**
     * @author iamacatfr
     * @reason fix crash on interact event with manuals from Openblocks by example if ars magica mod and witchery is
     *         present by disabling
    ...
    private boolean optimizationsAndTweaks$handleHowling(PlayerInteractEvent event, ExtendedPlayer playerEx) {
        if (event.entityPlayer.worldObj.isRemote && ((event.action == PlayerInteractEvent.Action.RIGHT_CLICK_AIR
            || event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK)
            && event.entityPlayer.rotationPitch == -90.0F
    ...
    private boolean optimizationsAndTweaks$handleHarvestingBlocks(PlayerInteractEvent event, ExtendedPlayer playerEx) {
        ...
        if (playerEx.getCreatureType() == TransformCreature.WOLF && playerEx.getWerewolfLevel() >= 3
            ...
            if (block == Blocks.grass || block == Blocks.sand
                ...
                || block == Blocks.gravel) {
                EntityAIDigBlocks
                    .tryHarvestBlock(event.world, event.x, event.y, event.z, event.entityPlayer, event.entityPlayer);
                event.setCanceled(true);
                return true;
            }
        } else if (playerEx.getVampireLevel() >= 6 && playerEx.getCreatureType() == TransformCreature.NONE
            && event.entityPlayer.isSneaking()
            && (event.entityPlayer.getHeldItem() == null || !Objects.requireNonNull(
                event.entityPlayer.getHeldItem()
                    .getItem())
                .func_150897_b(Blocks.stone))
            && event.entityPlayer.getFoodStats()
                .getFoodLevel() > 0) {
                    block = event.world.getBlock(event.x, event.y, event.z);
                    if (block == Blocks.stone || block == Blocks.netherrack || block == Blocks.cobblestone) {
                        EntityAIDigBlocks.tryHarvestBlock(
                            event.world,
                            event.x,
                            event.y,
                            event.z,
