 ...
 */
package fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.euclidean.oned;
...
public class IntervalsSet extends AbstractRegion<Euclidean1D, Euclidean1D> implements Iterable<double[]> {
    ...
    private static BSPTree<Euclidean1D> buildTree(final double lower, final double upper, final double tolerance) {
        if (Double.isInfinite(lower) && (lower < 0)) {
            ...
            final SubHyperplane<Euclidean1D> upperCut = new OrientedPoint(new Vector1D(upper), true, tolerance)
                .wholeHyperplane();
        ...
        final SubHyperplane<Euclidean1D> lowerCut = new OrientedPoint(new Vector1D(lower), false, tolerance)
            .wholeHyperplane();
        if (Double.isInfinite(upper) && (upper > 0)) {
    ...
    protected void computeGeometricalProperties() {
        if (getTree(false).getCut() == null) {
            ...
            if (Double.isInfinite(size)) {
                ...
                setBarycenter(
                    (Point<Euclidean1D>) ((OrientedPoint) getTree(false).getCut()
    ...
    public double getInf() {
        ...
        while (node.getCut() != null) {
            final OrientedPoint op = (OrientedPoint) node.getCut()
                .getHyperplane();
            inf = op.getLocation()
                .getX();
    ...
    private boolean isDirect(final BSPTree<Euclidean1D> node) {
        return ((OrientedPoint) node.getCut()
