 ...
 */
package fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.euclidean.twod;

import java.util.ArrayList;
import java.util.Collection;
import java.util.List;

import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.Point;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.euclidean.oned.Euclidean1D;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.euclidean.oned.Interval;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.euclidean.oned.IntervalsSet;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.euclidean.oned.Vector1D;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.AbstractRegion;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.AbstractSubHyperplane;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.BSPTree;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.BSPTreeVisitor;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.BoundaryAttribute;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.Hyperplane;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.Side;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.SubHyperplane;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.util.FastMath;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.util.Precision;
...
public class PolygonsSet extends AbstractRegion<Euclidean2D, Euclidean1D> {
     ...
     * calls to the {@link
     * fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.Region#checkPoint(fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.Point)
    ...
    private static Line[] boxBoundary(final double xMin, final double xMax, final double yMin, final double yMax,
        ...
        return new Line[] { new Line(minMin, maxMin, tolerance), new Line(maxMin, maxMax, tolerance),
            new Line(maxMax, minMax, tolerance), new Line(minMax, minMin, tolerance) };
    }
    ...
    private static void insertEdges(final double hyperplaneThickness, final BSPTree<Euclidean2D> node,
        ...
        for (final Edge edge : edges) {
            if (edge != inserted) {
                final double startOffset = inserted.getLine()
                    .getOffset(
                        (Point<Euclidean2D>) edge.getStart()
                            .getLocation());
                final double endOffset = inserted.getLine()
                    .getOffset(
                        (Point<Euclidean2D>) edge.getEnd()
                            .getLocation());
        ...
        if (vertices == null) {
            if (getTree(false).getCut() == null) {
                ...
                for (final List<Segment> loop : loops) {
                    if (loop.size() < 2 || (loop.size() == 2 && loop.get(0)
                        .getStart() == null
                        && loop.get(1)
                            .getEnd() == null)) {
                        // single infinite line
                        final Line line = loop.get(0)
                            .getLine();
                        vertices[i++] = new Vector2D[] { null,
                        ...
                        .getStart() == null) {
                            ...
                            for (Segment segment : loop) {
                                ...
                                if (j == 0) {
                                    ...
                                    double x = segment.getLine()
                                        .toSubSpace((Point<Euclidean2D>) segment.getEnd())
                                        .getX();
                                    x -= FastMath.max(1.0, FastMath.abs(x / 2));
                                    array[j++] = null;
                                    array[j++] = segment.getLine()
                                        .toSpace((Point<Euclidean1D>) new Vector1D(x));
                                }
                                ...
                                if (j == (array.length - 1)) {
                                    // last dummy point
                                    double x = segment.getLine()
                                        .toSubSpace((Point<Euclidean2D>) segment.getStart())
                                        .getX();
                                    x += FastMath.max(1.0, FastMath.abs(x / 2));
                                    array[j++] = segment.getLine()
                                        .toSpace((Point<Euclidean1D>) new Vector1D(x));
        ...
        for (final ConnectableSegment segment : segments) {
            if (segment.getNext() == null) {
                ...
                for (final ConnectableSegment candidateNext : segments) {
                    if (candidateNext.getPrevious() == null && candidateNext.getNode() == end
    ...
    private int splitEdgeConnections(final List<ConnectableSegment> segments) {
        ...
        for (final ConnectableSegment segment : segments) {
            if (segment.getNext() == null) {
                final Hyperplane<Euclidean2D> hyperplane = segment.getNode()
                    .getCut()
                    .getHyperplane();
                final BSPTree<Euclidean2D> end = segment.getEndNode();
                for (final ConnectableSegment candidateNext : segments) {
                    if (candidateNext.getPrevious() == null && candidateNext.getNode()
                        .getCut()
                        .getHyperplane() == hyperplane && candidateNext.getStartNode() == end) {
                        // connect the two segments
                        segment.setNext(candidateNext);
                        candidateNext.setPrevious(segment);
                        ++connected;
    ...
    private static class ConnectableSegment extends Segment {
         ...
         */
        ConnectableSegment(final Vector2D start, final Vector2D end, final Line line, final BSPTree<Euclidean2D> node,
            final BSPTree<Euclidean2D> startNode, final BSPTree<Euclidean2D> endNode) {
    ...
    private static class SegmentsBuilder implements BSPTreeVisitor<Euclidean2D> {
         ...
         */
        private void addContribution(final SubHyperplane<Euclidean2D> sub, final BSPTree<Euclidean2D> node,
            final Iterable<BSPTree<Euclidean2D>> splitters, final boolean reversed) {
            @SuppressWarnings("unchecked")
            ...
            for (final Interval i : intervals) {
                ...
                // find the 2D points
                final Vector2D startV = Double.isInfinite(i.getInf()) ? null
                    : (Vector2D) line.toSpace((Point<Euclidean1D>) new Vector1D(i.getInf()));
                final Vector2D endV = Double.isInfinite(i.getSup()) ? null
                    : (Vector2D) line.toSpace((Point<Euclidean1D>) new Vector1D(i.getSup()));
         ...
         */
        private BSPTree<Euclidean2D> selectClosest(final Vector2D point,
            final Iterable<BSPTree<Euclidean2D>> candidates) {

            BSPTree<Euclidean2D> selected = null;
            double min = Double.POSITIVE_INFINITY;

            for (final BSPTree<Euclidean2D> node : candidates) {
                final double distance = FastMath.abs(
                    node.getCut()
                        .getHyperplane()
                        .getOffset(point));
                if (distance < min) {
