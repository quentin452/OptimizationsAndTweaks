...
public abstract class AbstractSubHyperplane<S extends Space, T extends Space> implements SubHyperplane<S> {
    ...
    public AbstractSubHyperplane<S, T> applyTransform(final Transform<S, T> transform) {
        ...
        for (final Map.Entry<BSPTree<T>, BSPTree<T>> entry : map.entrySet()) {
            if (entry.getKey()
                .getCut() != null) {
                @SuppressWarnings("unchecked")
                BoundaryAttribute<T> original = (BoundaryAttribute<T>) entry.getKey()
                    .getAttribute();
                if (original != null) {
                    @SuppressWarnings("unchecked")
                    BoundaryAttribute<T> transformed = (BoundaryAttribute<T>) entry.getValue()
                        .getAttribute();
     ...
     * @return a new tree
     */
    private BSPTree<T> recurseTransform(final BSPTree<T> node, final Hyperplane<S> transformed,
        final Transform<S, T> transform, final Map<BSPTree<T>, BSPTree<T>> map) {
