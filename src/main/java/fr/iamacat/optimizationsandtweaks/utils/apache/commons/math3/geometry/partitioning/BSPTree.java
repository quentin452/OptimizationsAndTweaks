...
public class BSPTree<S extends Space> {
     ...
     * <p>
     * This method does <em>not</em> perform any verification on
    ...
    private SubHyperplane<S> fitToCell(final SubHyperplane<S> sub) {
        ...
        for (BSPTree<S> tree = this; tree.parent != null && s != null; tree = tree.parent) {
            if (tree == tree.parent.plus) {
                s = s.split(tree.parent.cut.getHyperplane())
                    .getPlus();
            } else {
                s = s.split(tree.parent.cut.getHyperplane())
                    .getMinus();
    ...
    public BSPTree<S> getCell(final Point<S> point, final double tolerance) {
        ...
        final double offset = cut.getHyperplane()
            .getOffset(point);
     ...
     * preserved, <em>no</em> such copy is done here!
     * </p>
     * <p>
     ...
     *         since all connections have already been established
     */
    private BSPTree<S> merge(final BSPTree<S> tree, final LeafMerger<S> leafMerger, final BSPTree<S> parentTree,
        ...
        if (cut == null) {
            ...
            merged.condense();
            if (merged.cut != null) {
                merged.cut = merged.fitToCell(
                    merged.cut.getHyperplane()
                        .wholeHyperplane());
            }
    ...
    public interface LeafMerger<S extends Space> {
         ...
         *         the arguments)
         */
        BSPTree<S> merge(BSPTree<S> leaf, BSPTree<S> tree, BSPTree<S> parentTree, boolean isPlusChild,
    ...
    public BSPTree<S> split(final SubHyperplane<S> sub) {
        ...
        switch (subParts.getSide()) {
            ...
            case BOTH: {
                ...
                final BSPTree<S> split = new BSPTree<S>(
                    sub,
     ...
     * @since 3.3
     */
    public BSPTree<S> pruneAroundConvexCell(final Object cellAttribute, final Object otherLeafsAttributes,
