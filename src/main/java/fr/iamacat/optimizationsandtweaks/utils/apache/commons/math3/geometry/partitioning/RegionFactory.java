 ...
 */
package fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning;
...
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.Space;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.BSPTree.VanishingCutHandler;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.Region.Location;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.geometry.partitioning.SubHyperplane.SplitSubHyperplane;
...
public class RegionFactory<S extends Space> {
        ...
        for (final Hyperplane<S> hyperplane : hyperplanes) {
            if (node.insertCut(hyperplane)) {
                ...
                for (BSPTree<S> tree = node; tree.getParent() != null && s != null; tree = tree.getParent()) {
                    final Hyperplane<S> other = tree.getParent()
                        .getCut()
                        .getHyperplane();
    ...
    public Region<S> union(final Region<S> region1, final Region<S> region2) {
        final BSPTree<S> tree = region1.getTree(false)
            .merge(region2.getTree(false), new UnionMerger());
    ...
    public Region<S> intersection(final Region<S> region1, final Region<S> region2) {
        final BSPTree<S> tree = region1.getTree(false)
            .merge(region2.getTree(false), new IntersectionMerger());
        tree.visit(nodeCleaner);
        return region1.buildNew(tree);
    }
    ...
    public Region<S> xor(final Region<S> region1, final Region<S> region2) {
        final BSPTree<S> tree = region1.getTree(false)
            .merge(region2.getTree(false), new XorMerger());
        tree.visit(nodeCleaner);
        return region1.buildNew(tree);
    }
    ...
    public Region<S> difference(final Region<S> region1, final Region<S> region2) {
        final BSPTree<S> tree = region1.getTree(false)
            .merge(region2.getTree(false), new DifferenceMerger(region1, region2));
        tree.visit(nodeCleaner);
        return region1.buildNew(tree);
    }
    ...
    private BSPTree<S> recurseComplement(final BSPTree<S> node) {
        ...
        for (final Map.Entry<BSPTree<S>, BSPTree<S>> entry : map.entrySet()) {
            if (entry.getKey()
                ...
                BoundaryAttribute<S> original = (BoundaryAttribute<S>) entry.getKey()
                    .getAttribute();
                if (original != null) {
                    @SuppressWarnings("unchecked")
                    BoundaryAttribute<S> transformed = (BoundaryAttribute<S>) entry.getValue()
                        .getAttribute();
                    for (final BSPTree<S> splitter : original.getSplitters()) {
                        transformed.getSplitters()
                            .add(map.get(splitter));
                    }
                }
            }
        }
    ...
    private class UnionMerger implements BSPTree.LeafMerger<S> {
        ...
        /** {@inheritDoc} */
        public BSPTree<S> merge(final BSPTree<S> leaf, final BSPTree<S> tree, final BSPTree<S> parentTree,
    ...
    private class DifferenceMerger implements BSPTree.LeafMerger<S>, VanishingCutHandler<S> {
        ...
        public BSPTree<S> fixNode(final BSPTree<S> node) {
            ...
            return new BSPTree<S>(
                region1.checkPoint(p) == Location.INSIDE && region2.checkPoint(p) == Location.OUTSIDE);
        }
