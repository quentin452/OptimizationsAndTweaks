...
public class Edge {
    ...
    public Vector3D getPointAt(final double alpha) {
        return circle.getPointAt(
            alpha + circle.getPhase(
                start.getLocation()
                    .getVector()));
    }
    ...
    void split(final Circle splitCircle, final List<Edge> outsideList, final List<Edge> insideList) {
        ...
        final double edgeStart = circle.getPhase(
            start.getLocation()
                .getVector());
        ...
        if (unwrappedEnd >= length - tolerance) {
            ...
            if (arcRelativeStart >= length - tolerance) {
                ...
                previousVertex = addSubEdge(
                    ...
                    arcRelativeStart - alreadyManagedLength,
                    outsideList,
                    splitCircle);
