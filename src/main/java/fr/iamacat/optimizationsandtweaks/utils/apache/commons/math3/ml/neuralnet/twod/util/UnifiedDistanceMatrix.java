...
package fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ml.neuralnet.twod.util;
...
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ml.neuralnet.twod.NeuronSquareMesh2D;
...
public class UnifiedDistanceMatrix implements MapVisualization {
     ...
     *                            {@link fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ml.neuralnet.SquareNeighbourhood
     *                            neighbourhood type}.
    ...
    private double[][] individualDistances(NeuronSquareMesh2D map) {
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            for (int j = 0; j < numCols; j++) {
                ...
                final double[] current = map.getNeuron(i, j)
                    .getFeatures();
                Neuron neighbour;

                // Right neighbour.
                neighbour = map.getNeuron(
                    i,
                    j,
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            for (int j = 0; j < numCols; j++) {
                ...
                final Neuron right = map.getNeuron(
                    i,
                    j,
                    NeuronSquareMesh2D.HorizontalDirection.RIGHT,
                    NeuronSquareMesh2D.VerticalDirection.CENTER);
                final Neuron bottom = map.getNeuron(
                    i,
                    j,
                    NeuronSquareMesh2D.HorizontalDirection.CENTER,
                    NeuronSquareMesh2D.VerticalDirection.DOWN);
                final Neuron bottomRight = map.getNeuron(
                    i,
                    j,
                    NeuronSquareMesh2D.HorizontalDirection.RIGHT,
                    NeuronSquareMesh2D.VerticalDirection.DOWN);

                final double current2BottomRight = bottomRight == null ? 0
                    : distance.compute(current.getFeatures(), bottomRight.getFeatures());
                final double right2Bottom = (right == null || bottom == null) ? 0
                    : distance.compute(right.getFeatures(), bottom.getFeatures());
