...
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.analysis.solvers.BracketedRealFieldUnivariateSolver;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.analysis.solvers.FieldBracketingNthOrderBrentSolver;
...
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ode.events.FieldEventHandler;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ode.events.FieldEventState;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ode.sampling.AbstractFieldStepInterpolator;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ode.sampling.FieldStepHandler;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.util.FastMath;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.util.IntegerSequence;
...
public abstract class AbstractFieldIntegrator<T extends RealFieldElement<T>> implements FirstOrderFieldIntegrator<T> {
    ...
    /** {@inheritDoc} */
    public void addEventHandler(final FieldEventHandler<T> handler, final double maxCheckInterval,
        ...
        addEventHandler(
            ...
            new FieldBracketingNthOrderBrentSolver<T>(
                field.getZero()
                    .add(DEFAULT_RELATIVE_ACCURACY),
                field.getZero()
                    .add(convergence),
                field.getZero()
                    .add(DEFAULT_FUNCTION_VALUE_ACCURACY),
                5));
    }

    /** {@inheritDoc} */
    public void addEventHandler(final FieldEventHandler<T> handler, final double maxCheckInterval,
        final double convergence, final int maxIterationCount, final BracketedRealFieldUnivariateSolver<T> solver) {
        eventsStates.add(
     ...
     */
    protected FieldODEStateAndDerivative<T> initIntegration(final FieldExpandableODE<T> eqn, final T t0, final T[] y0,
        final T t) {
     ...
     *                                       is called outside of a call to
     *                                       {@link #integrate(FieldExpandableODE, FieldODEState,
    ...
    protected FieldODEStateAndDerivative<T> acceptStep(final AbstractFieldStepInterpolator<T> interpolator,
        final T tEnd) throws MaxCountExceededException, DimensionMismatchException, NoBracketingException {
        ...
        SortedSet<FieldEventState<T>> occurringEvents = new TreeSet<FieldEventState<T>>(
        ...
        while (!occurringEvents.isEmpty()) {
            ...
            final FieldODEStateAndDerivative<T> eventState = restricted
                .getInterpolatedState(currentEvent.getEventTime());
            ...
            for (final FieldEventState<T> state : eventsStates) {
                ...
                if (newState != null) {
                    ...
                    final T[] y = equations.getMapper()
                        .mapState(newState);
                    final T[] yDot = computeDerivatives(newState.getTime(), y);
                    resetOccurred = true;
                    return equations.getMapper()
                        .mapStateAndDerivative(newState.getTime(), y, yDot);
                }
            }
        ...
        isLastStep = isLastStep || currentState.getTime()
            .subtract(tEnd)
            .abs()
            .getReal() <= FastMath.ulp(tEnd.getReal());
    ...
    protected void sanityChecks(final FieldODEState<T> eqn, final T t)
        ...
        final double threshold = 1000 * FastMath.ulp(
            FastMath.max(
                FastMath.abs(
                    eqn.getTime()
                        .getReal()),
                FastMath.abs(t.getReal())));
        final double dt = eqn.getTime()
            .subtract(t)
            .abs()
            .getReal();
        if (dt <= threshold) {
