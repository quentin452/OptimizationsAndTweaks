...
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ode.sampling.FieldStepInterpolator;
 ...
 * fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ode.nonstiff.AdaptiveStepsizeFieldIntegrator adaptive
...
public class ContinuousOutputFieldModel<T extends RealFieldElement<T>> implements FieldStepHandler<T> {
    ...
    public void append(final ContinuousOutputFieldModel<T> model)
        ...
        if (steps.size() == 0) {
            ...
            // safety checks
            final FieldODEStateAndDerivative<T> s1 = steps.get(0)
                .getPreviousState();
            final FieldODEStateAndDerivative<T> s2 = model.steps.get(0)
                .getPreviousState();
            ...
            final T current = lastInterpolator.getCurrentState()
                .getTime();
            final T previous = lastInterpolator.getPreviousState()
                .getTime();
            final T step = current.subtract(previous);
            final T gap = model.getInitialTime()
                .subtract(current);
            if (gap.abs()
                .subtract(
                    step.abs()
                ...
                throw new MathIllegalArgumentException(
                    LocalizedFormats.HOLE_BETWEEN_MODELS_TIME_RANGES,
                    gap.abs()
                        .getReal());
            }
        ...
        index = steps.size() - 1;
        finalTime = (steps.get(index)).getCurrentState()
            .getTime();
    ...
    public void handleStep(final FieldStepInterpolator<T> interpolator, final boolean isLast)
        ...
        if (steps.size() == 0) {
            initialTime = interpolator.getPreviousState()
                .getTime();
        ...
        if (isLast) {
            finalTime = interpolator.getCurrentState()
                .getTime();
            index = steps.size() - 1;
        }
    ...
    public FieldODEStateAndDerivative<T> getInterpolatedState(final T time) {
        ...
        T tMin = sMin.getPreviousState()
            .getTime()
            .add(
                sMin.getCurrentState()
                    .getTime())
            .multiply(0.5);

        int iMax = steps.size() - 1;
        final FieldStepInterpolator<T> sMax = steps.get(iMax);
        T tMax = sMax.getPreviousState()
            .getTime()
            .add(
                sMax.getCurrentState()
                    .getTime())
            .multiply(0.5);
        ...
        while (iMax - iMin > 5) {
            ...
            final T tMed = sMed.getPreviousState()
                .getTime()
                .add(
                    sMed.getCurrentState()
                        .getTime())
                .multiply(0.5);

            if (tMed.subtract(tMin)
                .abs()
                .subtract(1.0e-6)
                .getReal() < 0
                || tMax.subtract(tMed)
                ...
                final T iLagrange = dt2.multiply(dt3)
                    .multiply(d23)
                    .multiply(iMax)
                    .subtract(
                        dt1.multiply(dt3)
    ...
    private int locatePoint(final T time, final FieldStepInterpolator<T> interval) {
        if (forward) {
            if (time.subtract(
                interval.getPreviousState()
                    .getTime())
                .getReal() < 0) {
                return -1;
            } else if (time.subtract(
                interval.getCurrentState()
                    .getTime())
                .getReal() > 0) {
                    return +1;
                } else {
