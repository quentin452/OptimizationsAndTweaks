...
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.analysis.RealFieldUnivariateFunction;
...
public class FieldEventState<T extends RealFieldElement<T>> {
     ...
     */
    public FieldEventState(final FieldEventHandler<T> handler, final double maxCheckInterval, final T convergence,
        final int maxIterationCount, final BracketedRealFieldUnivariateSolver<T> solver) {
    ...
    public void reinitializeBegin(final FieldStepInterpolator<T> interpolator) throws MaxCountExceededException {
        ...
        if (g0.getReal() == 0) {
            ...
            final double epsilon = FastMath.max(
    ...
    public boolean evaluateStep(final FieldStepInterpolator<T> interpolator)
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (g0Positive ^ (gb.getReal() >= 0)) {
                ...
                increasing = gb.subtract(ga)
                    .getReal() >= 0;

                // find the event time making sure we select a solution just at or past the exact root
                final T root = forward ? solver.solve(maxIterationCount, f, ta, tb, AllowedSolution.RIGHT_SIDE)
                    : solver.solve(maxIterationCount, f, tb, ta, AllowedSolution.LEFT_SIDE);

                if (previousEventTime != null && root.subtract(ta)
                    .abs()
                    .subtract(convergence)
                    .getReal() <= 0
                    && root.subtract(previousEventTime)
                        .abs()
                        .subtract(convergence)
                        .getReal() <= 0) {
                    ...
                    do {
                        ...
                        ga = f.value(ta);
                    } while ((g0Positive ^ (ga.getReal() >= 0)) && (forward ^ (ta.subtract(tb)
                        .getReal() >= 0)));

                    if (forward ^ (ta.subtract(tb)
                        .getReal() >= 0)) {
                ...
                } else if (previousEventTime == null || previousEventTime.subtract(root)
                    .abs()
                    .subtract(convergence)
                    .getReal() > 0) {
                        pendingEventTime = root;
    ...
    public T getEventTime() {
        return pendingEvent ? pendingEventTime
            : t0.getField()
                .getZero()
                .add(forward ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY);
    }
    ...
    public void stepAccepted(final FieldODEStateAndDerivative<T> state) {
        ...
        if (pendingEvent && pendingEventTime.subtract(state.getTime())
            .abs()
            .subtract(convergence)
            .getReal() <= 0) {
