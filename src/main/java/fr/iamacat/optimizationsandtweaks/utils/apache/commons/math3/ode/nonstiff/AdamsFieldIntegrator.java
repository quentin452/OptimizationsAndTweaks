...
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.ode.MultistepFieldIntegrator;
...
public abstract class AdamsFieldIntegrator<T extends RealFieldElement<T>> extends MultistepFieldIntegrator<T> {
     ...
     */
    public AdamsFieldIntegrator(final Field<T> field, final String name, final int nSteps, final int order,
    ...
    public AdamsFieldIntegrator(final Field<T> field, final String name, final int nSteps, final int order,
        ...
        final double[] vecRelativeTolerance) throws IllegalArgumentException {
    ...
    /** {@inheritDoc} */
    @Override
    protected Array2DRowFieldMatrix<T> initializeHighOrderDerivatives(final T h, final T[] t, final T[][] y,
        final T[][] yDot) {
