...
public abstract class AdaptiveStepsizeFieldIntegrator<T extends RealFieldElement<T>>
     ...
     */
    public AdaptiveStepsizeFieldIntegrator(final Field<T> field, final String name, final double minStep,
    ...
    public void setStepSizeControl(final double minimalStep, final double maximalStep, final double absoluteTolerance,
        ...
        minStep = getField().getZero()
            .add(FastMath.abs(minimalStep));
        maxStep = getField().getZero()
            .add(FastMath.abs(maximalStep));
        initialStep = getField().getOne()
            .negate();
    ...
    public void setInitialStepSize(final T initialStepSize) {
        if (initialStepSize.subtract(minStep)
            .getReal() < 0
            || initialStepSize.subtract(maxStep)
                .getReal() > 0) {
            initialStep = getField().getOne()
                .negate();
        } else {
    ...
    public T initializeStep(final boolean forward, final int order, final T[] scale,
        final FieldODEStateAndDerivative<T> state0, final FieldEquationsMapper<T> mapper)
        ...
        T h = (yOnScale2.getReal() < 1.0e-10 || yDotOnScale2.getReal() < 1.0e-10) ? getField().getZero()
            .add(1.0e-6)
            : yOnScale2.divide(yDotOnScale2)
                .sqrt()
                .multiply(0.01);
        if (!forward) {
            h = h.negate();
        ...
        final T[] yDot1 = computeDerivatives(
            state0.getTime()
                .add(h),
            y1);
        ...
        for (int j = 0; j < scale.length; ++j) {
            final T ratioDotDot = yDot1[j].subtract(yDot0[j])
                .divide(scale[j]);
            yDDotOnScale = yDDotOnScale.add(ratioDotDot.multiply(ratioDotDot));
        }
        yDDotOnScale = yDDotOnScale.sqrt()
            .divide(h);

        // step size is computed such that
        // h^order * max (||y'/tol||, ||y''/tol||) = 0.01
        final T maxInv2 = MathUtils.max(yDotOnScale2.sqrt(), yDDotOnScale);
        final T h1 = maxInv2.getReal() < 1.0e-15 ? MathUtils.max(
            getField().getZero()
                .add(1.0e-6),
            h.abs()
                .multiply(0.001))
            : maxInv2.multiply(100)
                .reciprocal()
                .pow(1.0 / order);
        h = MathUtils.min(
            h.abs()
        ...
        h = MathUtils.max(
            ...
            state0.getTime()
                .abs()
                .multiply(1.0e-12)); // avoids cancellation when computing t1 - t0
    ...
    protected T filterStep(final T h, final boolean forward, final boolean acceptSmall)
        ...
        if (filteredH.subtract(maxStep)
            .getReal() > 0) {
            filteredH = maxStep;
        } else if (filteredH.add(maxStep)
            .getReal() < 0) {
