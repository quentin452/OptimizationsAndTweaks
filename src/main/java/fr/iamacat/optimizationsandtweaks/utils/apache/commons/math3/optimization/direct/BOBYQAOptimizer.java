...
// CHECKSTYLE: stop all
package fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.optimization.direct;
...
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.optimization.GoalType;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.optimization.MultivariateOptimizer;
import fr.iamacat.optimizationsandtweaks.utils.apache.commons.math3.optimization.PointValuePair;
...
public class BOBYQAOptimizer extends BaseAbstractMultivariateSimpleBoundsOptimizer<MultivariateFunction>
    ...
    private double bobyqb(double[] lowerBound, double[] upperBound) {
        ...
        for (int i = 0; i < n; i++) {
            trustRegionCenterOffset
                .setEntry(i, interpolationPoints.getEntry(trustRegionCenterInterpolationPointIndex, i));
            // Computing 2nd power
                    ...
                    if (trustRegionCenterInterpolationPointIndex != kbase) {
                        ...
                        for (int j = 0; j < n; j++) {
                            for (int i = 0; i <= j; i++) {
                                ...
                                gradientAtTrustRegionCenter.setEntry(
                                    i,
                                    gradientAtTrustRegionCenter.getEntry(i) + modelSecondDerivativesValues.getEntry(ih)
                    ...
                    if (dsq <= xoptsq * ONE_OVER_A_THOUSAND) {
                        ...
                        for (int j = 0; j < n; j++) {
                            ...
                            for (int k = 0; k < npt; k++) {
                                work1.setEntry(
                                    j,
                                    work1.getEntry(j) + modelSecondDerivativesParameters.getEntry(k)
                    ...
                    for (int m = 0; m < nptm; m++) {
                        ...
                        for (int k = 0; k < npt; k++) {
                            lagrangeValuesAtNewPoint
                                .setEntry(k, lagrangeValuesAtNewPoint.getEntry(k) + sum * zMatrix.getEntry(k, m));
                        }
                    }
                    dsq = ZERO;
                    double bsum = ZERO;
    ...
    private double[] altmov(int knew, double adelt) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (tmp > ZERO) {
                    ...
                    if (subd * tmp > upperDifference.getEntry(i) - trustRegionCenterOffset.getEntry(i)) {
                        // Computing MAX
                        subd = FastMath
                            .max(sumin, (upperDifference.getEntry(i) - trustRegionCenterOffset.getEntry(i)) / tmp);
                        iubd = i + 1;
                    }
                } else if (tmp < ZERO) {
                    ...
                    if (subd * tmp < lowerDifference.getEntry(i) - trustRegionCenterOffset.getEntry(i)) {
                        // Computing MAX
                        subd = FastMath
                            .max(sumin, (lowerDifference.getEntry(i) - trustRegionCenterOffset.getEntry(i)) / tmp);
                        iubd = -i - 1;
                    }
                }
            }
        ...
        for (int i = 0; i < n; i++) {
            ...
            newPoint
                .setEntry(i, FastMath.max(lowerDifference.getEntry(i), FastMath.min(upperDifference.getEntry(i), tmp)));
        }
        if (ibdsav < 0) {
        ...
        while (true) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (FastMath.min(trustRegionCenterOffset.getEntry(i) - lowerDifference.getEntry(i), glagValue) > ZERO
                    || FastMath.max(trustRegionCenterOffset.getEntry(i) - upperDifference.getEntry(i), glagValue)
                        < ZERO) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (work1.getEntry(i) == bigstp) {
                    ...
                    final double min = FastMath
                        .min(upperDifference.getEntry(i), trustRegionCenterOffset.getEntry(i) + work1.getEntry(i));
            ...
            if (curv > -gw && curv < -gw * (ONE + FastMath.sqrt(TWO))) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    alternativeNewPoint.setEntry(
                        i,
                        FastMath.max(lowerDifference.getEntry(i), FastMath.min(upperDifference.getEntry(i), tmp)));
                }
                // Computing 2nd power
        ...
        do {
            ...
            for (int j = 0; j < n; j++) {
                currentBest.setEntry(
                    j,
                    FastMath.min(
                        FastMath.max(lowerBound[j], originShift.getEntry(j) + interpolationPoints.getEntry(nfm, j)),
                        upperBound[j]));
            ...
            if (numEval <= 2 * n + 1) {
                if (numEval >= 2 && numEval <= n + 1) {
                    ...
                    gradientAtTrustRegionCenter
                        .setEntry(nfxm, (gradientAtTrustRegionCenter.getEntry(nfxm) * stepb - tmp * stepa) / diff);
                    ...
                    // zMatrix.setEntry(nfm, nfxm, FastMath.sqrt(HALF) * recip); // XXX "testAckley" and "testDiffPow"
                    // fail.
    ...
    private double[] trsbox(double delta, ArrayRealVector gnew, ArrayRealVector xbdi, ArrayRealVector s,
        ...
        int iterc;
        double resid = 0, delsq = 0, ggsav = 0, tempa = 0, tempb = 0, redmax = 0, dredsq = 0, redsav = 0, gredsq = 0,
            rednew = 0;
        int itcsav = 0;
        ...
        for (;;) {
            switch (state) {
                ...
                case 210: {
                    ...
                    final RealVector tmp = interpolationPoints.operate(s)
