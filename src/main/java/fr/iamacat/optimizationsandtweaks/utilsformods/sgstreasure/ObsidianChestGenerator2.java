...
public class ObsidianChestGenerator2 {
    ...
    public static Coords getNearestChestCoords(World world, int x, int y, int z) {
        List<PlaceOfInterest> chestsByCategory = Treasure.getPoiRegistry()
            .getByCategory(PoiType.CHEST.getName());
                    ...
                    do {
                        ...
                        do {
                            ...
                            do {
                                if (!var8.hasNext()) {
                                    if (nearestPoi == null) {
                                        Treasure.logger.info(
                                            "Unable to located a chest of type PIRATE_CHEST|DREAD|WITHER_CHEST. Placing in the first chest in POI.");
                        ...
                        } while (!chestTypes.contains(
                            chest.getChestType()
                                .getName()));

                        te = (AbstractTreasureChestTileEntity) world.getTileEntity(
                            poi.getCoords()
                                .getX(),
                            poi.getCoords()
                                .getY(),
                            poi.getCoords()
                                .getZ());
                    } while (te.getProxy() != null && te.getProxy()
                        .func_70301_a(0) != null
                        && te.getProxy()
                            .func_70301_a(0)
                            .getItem() instanceof UniqueKeyItem);

                    ChunkCoordinates chunkCoords = new ChunkCoordinates(x, y, z);
                    distance = chunkCoords.getDistanceSquared(
                        poi.getCoords()
                            .getX(),
                        poi.getCoords()
                            .getY(),
