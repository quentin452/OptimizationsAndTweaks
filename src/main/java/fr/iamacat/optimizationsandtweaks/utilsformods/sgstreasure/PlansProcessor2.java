...
public class PlansProcessor2 {
        ...
        for (int mapY = 0; mapY < plans.getHeight(); ++mapY) {
            for (int mapZ = 0; mapZ < plans.getDepth(); ++mapZ) {
                for (int mapX = 0; mapX < plans.getWidth(); ++mapX) {
                    PlansBlockRule rule = plans.getRules()
                        .get(plans.getBlockMap()[mapY][mapZ][mapX]);
                    Plans.log.trace("Got rule " + rule);
                    int yOffset = y + plans.getVerticalOffset();
                    if (!plans.getPostProcessBlocks()
                        .isEmpty() && plans.getPostProcessBlocks()
                            .contains(rule.getBlockName())) {
                    ...
                    } else if (!plans.getPostProcess()
                        .isEmpty() && plans.getPostProcess()
                            .contains(rule.getBlockId())) {
    ...
    private void processBlock(World world, Random random, int x, int y, int z, PlansBlockRule rule, Plans plans) {
        ...
        byte meta = 0;
        Block worldBlock;
        if ((rule.getBlockName()
            .equals("air") || rule.getBlockId() == 0) && !plans.getPreservePlansAir()) {
            Plans.log.trace("Equals air and preserving air");
        } else if (!plansProcessor.isEnableSpawners() && rule.getBlockName()
            .equals("mob_spawner")) {
                Plans.log.info("Block is a SPAWNER, returning without generating - gen air instead");
                world.setBlock(x, y, z, Blocks.air, 0, 2);
            } else if (rule.getProbability() == 100 && (rule.getBlockName()
                .equals("null_block")
                || rule.getSubstituteName()
                    .equals("null_block")
                || rule.getBlockId() == -1000
                || rule.getSubstituteId() == -1000)) {
                    Plans.log.trace("Equals Null block");
                } else if (plans.getSubstituteWithAirByName() != null && !plans.getSubstituteWithAirByName()
                    .isEmpty()
                    && plans.getSubstituteWithAirByName()
                        .contains(rule.getBlockName())) {
                            world.setBlock(x, y, z, Blocks.air, 0, 2);
                            Plans.log.trace("Substituting for air");
                        } else
                    if (plans.getSubstituteWithAir() != null && !plans.getSubstituteWithAir()
                        .isEmpty()
                        && plans.getSubstituteWithAir()
                            .contains(rule.getBlockId())) {
                                world.setBlock(x, y, z, Blocks.air, 0, 2);
                                Plans.log.trace("Substituting for air");
                            } else {
                                ...
                                if (plans.getPreserveWorldWater() && worldBlock == Blocks.water) {
                                    ...
                                    if (rule.getProbability() != 100 && random.nextInt(100) > rule.getProbability()) {
                                        ...
                                        if (worldBlock == conditionBlock
                                            && world.getBlockMetadata(x, y, z) == conditionMeta) {
                                    ...
                                    if (block == null) {
                                        Plans.log.trace(
                                            "Unable to get block for rule " + rule.getRuleId()
                                                + ". Block will not be processed. Moving to next block.");
                                    } else if (!rule.getRequiresSupport() || this.isSupportingBlock(world, x + 1, y, z)
                                        || this.isSupportingBlock(world, x - 1, y, z)
                                        || this.isSupportingBlock(world, x, y + 1, z)
                                        || this.isSupportingBlock(world, x, y - 1, z)
                                        || this.isSupportingBlock(world, x, y, z + 1)
                                        || this.isSupportingBlock(world, x, y, z - 1)) {
    ...
    private void processSpecials(World world, Random random, int x, int y, int z, Block block, PlansBlockRule rule,
        Plans plans) {
        if (block == Blocks.mob_spawner) {
            String spawns = "Zombie";
            if (rule.getSpawns() != null && !rule.getSpawns()
                .isEmpty()) {
                spawns = rule.getSpawns()
                    .get(
                        random.nextInt(
                            rule.getSpawns()
                                .size()));
            ...
            if (inventory != null) {
                ...
                if (chest != null) {
                    ...
                    for (InventoryItem inventoryItem : inventory.getItems()) {
                        ItemStack itemStack;
                        if (inventoryItem.getName() != null && !inventoryItem.getName()
                            .isEmpty()) {
                            itemStack = new ItemStack(
                                (Item) Item.itemRegistry.getObject(inventoryItem.getName()),
                                inventoryItem.getCount());
                        } else {
                            itemStack = new ItemStack(
                                Item.getItemById(inventoryItem.getId()),
                                inventoryItem.getCount());
                        }
    ...
    public Block getProbabilitySubBlockFromRule(PlansBlockRule rule) {
        Block block = null;
        if (rule.getProbabilitySubName() != null && !rule.getProbabilitySubName()
            .isEmpty()) {
    ...
    public Block getBlockFromRule(PlansBlockRule rule) {
        Block block;
        if (rule.getSubstituteName() != null && !rule.getSubstituteName()
            .isEmpty()) {
            Plans.log.trace("Getting substitute by name:" + rule.getSubstituteName());
            if (rule.getSubstituteName()
                .equals("null_block")) {
                return null;
            }
        ...
        } else if (rule.getBlockName() != null && !rule.getBlockName()
            .equals("")) {
